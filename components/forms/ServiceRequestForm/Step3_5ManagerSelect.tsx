...
interface Step3_5ManagerSelectProps {
...
export default function Step3_5ManagerSelect({ data, onUpdate, onNext, onPrev }: Step3_5ManagerSelectProps) {
        ...
        {data.designatedManager && (
          <div className="rounded-lg border-2 border-primary bg-blue-50 p-4">
            <div className="flex items-start gap-4">
              <img
                src={data.designatedManager.photo || defaultAvatar}
                alt={data.designatedManager.name}
                className="w-16 h-16 rounded-full object-cover bg-gray-200"
                onError={(e) => { (e.target as HTMLImageElement).src = defaultAvatar }}
              />
              <div className="flex-1">
                <p className="font-semibold text-lg">{data.designatedManager.name}</p>
                <p className="text-sm text-gray-600">{data.designatedManager.phone}</p>
                <p className="text-sm text-gray-600">{data.designatedManager.address}</p>
                <p className="text-sm text-primary mt-1">{data.designatedManager.specialty}</p>
              </div>
              <button
                type="button"
