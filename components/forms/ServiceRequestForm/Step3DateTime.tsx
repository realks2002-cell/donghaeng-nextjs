...
import { ServiceRequestFormData, TIME_OPTIONS, DURATION_OPTIONS, calculatePrice } from './types'
...
export default function Step3DateTime({ data, onUpdate, onNext, onPrev }: Step3DateTimeProps) {
  // 최소 날짜: 오늘
  const minDate = new Date().toISOString().split('T')[0]

  const estimatedPrice = calculatePrice(data.serviceType, data.durationHours)
  const pricePerHour = data.serviceType ? SERVICE_TYPES[data.serviceType as ServiceType]?.pricePerHour ?? 0 : 0
      ...
      <div className="mt-4 space-y-4">
            ...
            {DURATION_OPTIONS.map((hours) => (
              ...
              >
                <input
                  ...
                  name="duration_hours"
                  value={hours}
                  checked={data.durationHours === hours}
                  onChange={() => onUpdate({ durationHours: hours })}
                  className="sr-only"
                />
                {hours}시간
              </label>
        ...
        <div className="rounded-lg bg-gray-50 p-4">
          ...
          <p className="mt-1 text-xs text-gray-500">
            선택한 서비스 요금 {pricePerHour.toLocaleString()}원/시간 × {data.durationHours || 0}시간 · 최종 금액은 실제 소요 시간에 따라 달라질 수 있습니다.
          </p>
        </div>
      </div>
